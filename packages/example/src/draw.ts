...
export function createShapes(paperScope: paper.PaperScope) {
	...
	// 创建三个图层
	const geometryLayer = new paperScope.Layer();
	geometryLayer.name = '几何图形图层';

	geometryLayer.activate();

	const pathsLayer = new paperScope.Layer();
	pathsLayer.name = '路径和线条图层';

	const textEffectsLayer = new paperScope.Layer();
	...
	// 圆形
	const circle1 = new paperScope.Path.Circle({
		center: [120, 120],
		radius: 40,
		fillColor: '#ff6b6b',
		strokeColor: '#dc3545',
	...
	// 椭圆
	const ellipse = new paperScope.Path.Ellipse({
		point: [320, 70],
		size: [100, 60],
		fillColor: '#ffe66d',
		strokeColor: '#ffc107',
	...
	// 圆角矩形
	const roundRect = new paperScope.Path.Rectangle({
		point: [450, 80],
		size: [80, 80],
		radius: 15,
		fillColor: '#a855f7',
	...
	// 多边形 (六边形)
	const hexagon = new paperScope.Path.RegularPolygon({
		center: [250, 320],
		sides: 6,
		radius: 35,
		fillColor: '#6f42c1',
	...
	triangle.fillColor = new paperScope.Color('#fd7e14');
	triangle.strokeColor = new paperScope.Color('#e8590c');
	triangle.strokeWidth = 2;
	triangle.closed = true;
	triangle.name = 'Orange Triangle';

	// 渐变图形
	const gradientCircle = new paperScope.Path.Circle({
		center: [600, 120],
		radius: 40,
		fillColor: {
			gradient: {
	...
	// 折线路径
	const zigzag = new paperScope.Path();
	zigzag.strokeColor = new paperScope.Color('#fd7e14');
	...
	// 贝塞尔曲线
	const curve = new paperScope.Path();
	curve.strokeColor = new paperScope.Color('#e83e8c');
	curve.strokeWidth = 4;
	...
	// 星形 (手绘路径)
	const star = new paperScope.Path();
	star.fillColor = new paperScope.Color('#20c997');
	star.strokeColor = new paperScope.Color('#17a2b8');
	star.strokeWidth = 2;
	...
	const center = new paperScope.Point(120, 320);
	...
	// 图案填充路径
	const patternRect = new paperScope.Path.Rectangle({
		point: [600, 200],
		size: [80, 60],
		strokeColor: '#495057',
		strokeWidth: 2
	...
	// 点状图案组
	const patternGroup = new paperScope.Group();
	for (let x = 0; x < 8; x++) {
		for (let y = 0; y < 6; y++) {
			if ((x + y) % 2 === 0) {
	...
	// 副标题
	const subtitle = new paperScope.PointText({
		point: [450, 320],
		content: 'DevTools 测试',
		fillColor: '#6c757d',
		fontFamily: 'Arial',
	...
	// 图层说明文本
	const layerInfo = new paperScope.PointText({
		point: [50, 50],
		content: '图层 1: 几何图形\n图层 2: 路径线条\n图层 3: 文本特效',
		fillColor: '#495057',
		fontFamily: 'Arial',
	...
	// 组合图形
	const shapeGroup = new paperScope.Group();
	shapeGroup.name = 'Geometric Shapes Group';

	// 在组中添加一些小图形
	for (let i = 0; i < 4; i++) {
		const smallShape = new paperScope.Path.Circle({
	...
	// 带透明度的图形
	const transparentShape = new paperScope.Path.Rectangle({
		point: [50, 400],
		size: [150, 80],
		fillColor: '#17a2b8',
		opacity: 0.6
	...
	// 重叠的透明圆形
	const overlappingCircles = new paperScope.Group();
	...
	// 发光效果文本
	const glowText = new paperScope.PointText({
		point: [400, 480],
		content: '✨ 特殊效果文本 ✨',
		fillColor: '#ffd700',
		fontFamily: 'Arial',
	...
	shadowText.fillColor = new paperScope.Color('#333333');
